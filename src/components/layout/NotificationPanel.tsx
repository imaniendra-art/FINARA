  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="relative flex h-10 w-10 items-center justify-center rounded-full bg-slate-50 border border-slate-100 text-slate-600 transition hover:bg-slate-100 hover:text-slate-900 active:scale-95 cursor-pointer"
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 mt-3 w-80 sm:w-96 overflow-hidden rounded-[24px] border border-slate-100 bg-white shadow-[0_12px_40px_rgba(0,0,0,0.08)] z-50 transition-all duration-200 animate-in fade-in-0 zoom-in-95 origin-top-right">
            {/* Header */}
              ...
              {alerts.length > 0 ? (
                alerts.map((alert) => (
                  ...
                  >
                    ...
                    <div>
                      <p className="font-bold text-slate-800 leading-tight">{alert.title}</p>
                      <p className="mt-1 text-slate-500 leading-relaxed font-medium">{alert.desc}</p>
                    </div>
                  </div>
                ))
              ) : (
